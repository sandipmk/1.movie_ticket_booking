...
$db = mysqli_select_db($con, "cinema_db");
	...
	$ORDERID = isset($_GET['ORDERID']) ? $_GET['ORDERID'] : '';
		...
		$qry = "select * from bookingtable where ORDERID = '" . $_GET['ORDERID'] . "'";
		// if ((!$_GET['ORDERID'])) {
	   ...
	   if ($row = mysqli_fetch_assoc($result)) {
		   ...
		   $movie_date = $row['bookingDate'];
		   $theatre = $row['bookingTheatre'];
		   $type = $row['bookingType'];
		   $time = $row['bookingTime'];
		   $amount = $row['amount'];
		   $ORDERID = $row['ORDERID'];
		   $invoice_date = isset($row['DATE-TIME']) ? $row['DATE-TIME'] : date('Y-m-d H:i:s');
	   } else {
		   // No data found for ORDERID
		   echo "<script>alert('No booking found for this ORDERID');window.location.href='index.php';</script>";
		   exit;
	   }
						...
						<tr>
							<td class="title">
								<div class="footer-brand">
									<h1 class="footer-heading">Gujarat Cinema</h1>
								</div>
							</td>
							<td>
							   Invoice #: <?php echo $ORDERID; ?><br>
							   Created: <?php date_default_timezone_set('Asia/Kolkata');
										   echo date("d-m-y h:i:s", strtotime($invoice_date));  ?><br>
